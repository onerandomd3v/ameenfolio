...
4. Provision Neon Auth, enable GitHub only, and configure its callback URL for local, preview, and production origins. Set a random cookie secret of at least 32 characters. Set `ADMIN_GITHUB_USER_ID` to the numeric GitHub provider account ID for the owner; keep it only in `.env.local` or deployment secrets.
